...
class SegSAMPLerEmbMLP(BasePLer):
    def __init__(self,
                 backbone,
        ...
        torch.nn.init.xavier_normal_(self.project_head[0].weight.data)
        torch.nn.init.xavier_normal_(self.project_head[2].weight.data)
        ...
        for idx in range(len(results)):
            ...
            for jdx,contour in enumerate(one_contours):
                ...
                if abs(cv2.contourArea(contour)) > 100*100 and  contour_box[2]>100 and contour_box[3]>100:
                    ...
                    sparse_embeddings=torch.cat((F.normalize(sparse_embeddings,dim=2), prompt_emb[idx].reshape(1,-1).unsqueeze(1).expand(-1, 2, -1)), dim=2)
